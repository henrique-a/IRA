var butcalcula = document.createElement("BUTTON");
var t = document.createTextNode("Calcular IRA");
butcalcula.setAttribute("id", "btncalcula");
...
$("document").ready(function (){
	...
	if($("#i").val().length != 0 && $("#T").val().length != 0 && $("#C").val().length != 0){
		...
		$("<p>*Nas disciplinas anuais, serÃ¡ considerado o semestre de inÃ­cio delas.</p>").appendTo("#aviso");
		...
		butcalcula.appendChild(t);
		document.getElementById('calcula').appendChild(butcalcula);
...
$('nota1').mask('00000-000', options);
        ...
        butcalcula.onmouseover = function (){
		this.setAttribute("class", "btnon");
	}

        butcalcula.onmouseout = function (){
		this.setAttribute("class", "btnoff");
	}

	butcalcula.onclick = function (){
	var T = $("#T").val();
	var C = $("#C").val();
